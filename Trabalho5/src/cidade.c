...
Reta* qry_bombaRadiacaoNuc(Cidade cid, double x, double y, char* nomeSvg, char* nomeTxt, char* arqPol, char* caminhoDoArquivo){
    cidade *city = cid;
    Tree retas = criaArvore(comparaReta);
    percorreArvore(city->arvoreMuro, qry_bombaRadiacaoArvoreMuros, retas);
    int tamPol=0;
    ...
    int tamBomba;
    Ponto* bombaPontos = qry_bombaRadiacao(x, y, nomeSvg, retas, "green", &tamBomba);
    
    Reta* bomba = malloc(retaSizeof()*tamBomba);
    for(int i=1;i<tamBomba;i++){
        bomba[i] = criarReta(getPontoX(bombaPontos[i-1]), getPontoY(bombaPontos[i-1]),
                       getPontoX(bombaPontos[i]), getPontoY(bombaPontos[i]));
        pontoFinalizar(bombaPontos[i-1]);
    }
